...
// 매일 출연한 가수의 점수가 지금까지 출연 가수들의 점수 중 상위 k번째 이내이면 해당 가수의 점수를 명예의 전당이라는
// 목록에 올려 기념합니다.
//
...
fun main() {
    ...
    solution11(3, intArrayOf(10, 100, 20, 150, 1, 100, 200))
}

fun solution11(k: Int, score: IntArray): IntArray {
    var answer: IntArray = intArrayOf()
    var list = arrayListOf<Int>()
    for (i in 0 until score.size) {
        list.add(score[i])
        list.sortDescending()
        answer += if (list.lastIndex < k) list[list.lastIndex] else list[k-1]
    }
    return answer
}