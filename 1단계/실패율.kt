...
fun main() {
    solution13(5, intArrayOf(2, 1, 2, 6, 2, 4, 3, 3))
}
...
fun otherSolution(n: Int, stages: IntArray): IntArray {
    ...
    return Array<Stage>(n){
        // 실패 항목 카운트
        val fail:Double = stages.count{ s -> s == it + 1 }.toDouble()
        // 게임 횟수 카운트
        val total:Double = stages.count{ s -> s >= it + 1 }.toDouble()
        // 실패가 0일 경우 0으로 초기화, 실패율이 계산 가능한 경우 계산 후 추가
