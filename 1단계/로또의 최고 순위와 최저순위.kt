...
//재사용 한부분
//.intersect()함수는 Collection 타입에서 사용할수 있기 때문에 .toSet()으로 형변환을 해줘야한다.
fun main() {
...
//    solution(intArrayOf(45, 4, 35, 20, 3, 9), intArrayOf(20, 9, 3, 45, 4, 35))
}


fun solution(lottos: IntArray, win_nums: IntArray): IntArray {
    val count = lottos.count { it == 0 }
    val overlap = lottos.toSet().intersect(win_nums.toSet()).toIntArray().size
    return intArrayOf(ranking(count + overlap), ranking(overlap))
}

fun ranking(sum: Int): Int {
    when (sum) {
        2 -> {
            return 5
        }
...
fun otherSolution(lottos: IntArray, winNums: IntArray): IntArray {
    return intArrayOf(
